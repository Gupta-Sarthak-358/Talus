"""Darjeeling-corridor NGEN extraction (SIH26001, closes the Darjeeling data gap).

Mirrors the Gangtok S1-S4 pipeline with identical methods, new coordinates.
Zone coords are read from data/sih26001/fixtures/slopes.darjeeling.json
...
Pedigree per feature (same bars as Gangtok):
  ...
  USGS DEM-6 ... LOCAL tile data/raw/dem/n27_e088_1arc_v3.tif (Darjeeling
                 ...
                 density on a Darjeeling catchment window (REAL)
  CCI soil ..... LOCAL data/raw/soil/C3S-*.nc June 10-16 2024, same flag-mask
                 method as extract_soil_cci.py, per-D-cell mean (REAL)
  WorldCover ... SAME tile N27E087 (27-30N/87-90E covers Darjeeling),
                 rasterio /vsicurl/ 3x3 mode (REAL, network)
  Sentinel-2 ... Element84 STAC least-cloudy 2024 scene over Darjeeling bbox
                 + COG red/NIR/SCL reads (REAL, network)
  OSM .......... Overpass via extract_s1_osm.py driver, same filters/radii
                 (osm-qa-unverified, network)
  lithology .... darjeeling_gneiss PROXY-published-map (Gangtok DRAP p71/p118
                 names Darjeeling Gneiss as the regional unit; limit stated)
  lineament .... 0.8 km/km2 PROXY-regional (same Himalayan-50K literature
                 basis; Darjeeling figure NOT consulted, limit stated)
  labels ....... LOCAL GSI shapefile WEST BENGAL rows, 300 m rule, event=0
...
Run (system py311 has xarray+netCDF4+rasterio+numpy):
  python scripts/extract_darjeeling_ngen.py --only local
  python scripts/extract_darjeeling_ngen.py --only network
  python scripts/extract_darjeeling_ngen.py   (all)
"""
...
DARJ_BBOX = [88.20, 26.99, 88.32, 27.08]
DARJ_CATCH = {"lat0": 27.00, "lat1": 27.07, "lon0": 88.23, "lon1": 88.29}
LABEL_BBOX = (88.15, 88.35, 26.95, 27.10)
BUFFER_M = 300.0
KILL_BITS = 4 | 8 | 16 | 32
...
def phase_imd() -> dict:
    ...
    print(f"[OK] Darjeeling peak window end={peak.date()} 24h={day:.1f} 7d={w7:.1f} 30d={w30:.1f}")
...
def usgs_accumulation():
    ...
    r1 = int((north - DARJ_CATCH["lat1"]) / res)
    r2 = int((north - DARJ_CATCH["lat0"]) / res)
    c1 = int((DARJ_CATCH["lon0"] - west) / res)
    c2 = int((DARJ_CATCH["lon1"] - west) / res)
    z = full[r1:r2, c1:c2].copy()
    void = (z == ds.nodata)
    print(f"[OK] Darjeeling window voids: {int(void.sum())} - filling by neighbour mean")
    ...
    z = zm
    H, W = z.shape
    dxm = 111320.0 * math.cos(math.radians(27.04)) * res
    filled = z.copy()
    ...
    for y, x in zip(ys.tolist(), xs.tolist()):
        if has[y, x]:
            ...
            if ry != y or cx != x:
                acc[ry, cx] += acc[y, x]
    print(f"[OK] Darjeeling accumulation done; max cells = {int(acc.max())}")
    return ds, full, res, west, north, grid_xy, acc, r1, c1, dxm
...
def phase_usgs() -> dict:
    ...
    for zid, (la, lo) in ZONES.items():
        ...
        curv = d2x + d2y
        assert 1500.0 <= elev <= 2600.0, f"{zid} elev {elev} outside Darjeeling plausibility"
...
def gsi_load():
    import numpy as np
    tmp = Path(tempfile.mkdtemp(prefix="gsi_darj_"))
...
def phase_labels() -> dict:
    ...
    a = gsi_load()
    wb = np.array([("WEST BENGAL" in clean(x).upper()) for x in a["STATE"]])
    sub = a[wb]
    print(f"[OK] WB rows: {len(sub)}")
    ...
    print(f"[OK] WB rows with coords: {len(sub)}")
    ...
    with (EVID / "darjeeling_bbox_sample.csv").open("w", encoding="utf-8", newline="") as f:
        f.write(",".join(cols) + "\n")
        for row in sample:
            vals = []
            for c in cols:
            ...
            f.write(",".join(vals) + "\n")
    print(f"[OK] Darjeeling-bbox sample: {len(sample)} rows")
    join, rows = {}, {}
    for zid, (la, lo) in ZONES.items():
    ...
    (EVID / "darjeeling_join.json").write_text(json.dumps(
...
def phase_static() -> dict:
    ...
    # Gangtok extract_lithology/extract_lineament, Darjeeling-specific basis).
    return {
        "lithology": {
            zid: "darjeeling_gneiss" for zid in ZONES
        },
        "lithology_basis": ("Gangtok DRAP p71/p118 names Darjeeling Gneiss as the regional unit "
                            "alongside Lingste/Chungthang; Darjeeling town stands on Darjeeling Gneiss "
                            "per GSI regional geology. PROXY-published-map, not a Bhukosh vector clip."),
        ...
        "lineament_basis": ("Same Himalayan-50K literature basis as Gangtok (0.3-1.4, conservative 0.8); "
                            "Darjeeling figure NOT consulted. PROXY-regional; Bhuvan WB-clip "
                            "is the upgrade path."),
    }
...
def phase_sentinel() -> dict:
    ...
    body = json.dumps({
        "collections": ["sentinel-2-l2a"], "bbox": DARJ_BBOX,
    ...
    if not feats:
        raise RuntimeError("STAC returned no scenes for Darjeeling bbox/year")
    feats.sort(key=lambda f: f["properties"].get("eo:cloud_cover", 99))
    # The least-cloudy scene's granule may not cover all D points (D3/D4 fell
    # outside the Gangtok granule). Pick the first candidate whose red COG
    # contains every D zone - single-scene consistency preserved.
    import rasterio
    ...
    for cand in feats:
        ...
        if ok_all:
            ...
            break
    if item is None:
        raise RuntimeError("no 2024 scene covers all D1-D4 in one granule - not shipping split-scene NDVI")
    props = item["properties"]
    pid, date, cloud = item.get("id", ""), str(props.get("datetime", ""))[:10], props.get("eo:cloud_cover")
    print(f"[OK] scene {pid} date={date} cloud={cloud}% (covers all D)")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Darjeeling NGEN extraction")
    ...
    print()
    print("=== DARJEELING ROW VALUES (review before CSV freeze) ===")
    imd = merged.get("imd", {})
    ...
    for zid in ZONES:
        ...
        print(f"{zid}: slope={us.get('slope_angle')} elev={us.get('elevation')} asp={us.get('aspect')} "
              ...
              f"lith=darjeeling_gneiss road={os_.get('distance_to_road')} river={os_.get('distance_to_river')} "
