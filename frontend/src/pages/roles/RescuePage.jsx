...
export default function RescuePage() {
  const { setRole, activeRoutePlan } = useTalusContext();
  useEffect(() => setRole('rescue_team'), []);
  return (
    <main className="max-w-[1920px] mx-auto px-3 sm:px-4 py-4 space-y-4">
      <div className="bg-risk-critical/10 border border-risk-critical/30 rounded-xl px-4 py-2 text-xs font-bold text-risk-critical">NDRF / SDRF Rescue Team — Strictly bypass R2, ingress via Tadong Valley</div>
