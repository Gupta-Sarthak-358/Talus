  ...
  return (
    <main className="max-w-[1920px] mx-auto px-3 sm:px-4 py-4 space-y-4">
      <div className="bg-mine-darker border border-mine-border rounded-xl px-4 py-2 flex flex-wrap items-center justify-between gap-2 text-xs">
        <span className="font-bold text-mine-text">{t('role.district_officer')} {t('district.closure')}</span>
        <span className="text-mine-muted">{zones.length} {t('district.sub')}</span>
      </div>
      {/* Officer primary workflow first — queue above map */}
      <div className="flex gap-2">
        <Link to="/reports" className="flex-1 py-2 bg-talus-600 text-white rounded-lg text-xs font-bold text-center flex items-center justify-center gap-1"><FileText className="w-3.5 h-3.5" /> {t('district.review_queue')}</Link>
        <Link to="/reports" className="px-3 py-2 bg-mine-darker border border-mine-border rounded-lg text-xs font-semibold flex items-center gap-1"><Bell className="w-3.5 h-3.5" /> {t('district.multilang')}</Link>
      </div>
