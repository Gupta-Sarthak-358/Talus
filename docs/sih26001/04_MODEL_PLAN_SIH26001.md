...
1. **Spatial cross-validation:** `KMeans-8` on coords → `GroupKFold(8)` OOF `train_sih26001.py:129` — `RF 0.8983 XGB 0.9029 LGBM 0.9015` `metrics.md:9`, per-cluster `cluster_0 n/a` single-class logged `metrics.md:25`. Random splits banned (v1 leakage proof).
2. **Temporal validation:** `≤2018 vs ≥2019` with `≥30 dated/side` rule `train_sih26001.py:54` — rescued year `61` clusters `build_training_matrix.py:294` → `673/73 dated` `done:true` `RF test AUC 0.8189 Brier 0.1216` `manifest.training.json:144`.
3. **Calibration:** isotonic on `RF` OOF `Brier 0.118 ECE 0.0` vs `naive 0.25` `calibration.md:8` (same-OOF optimism caveat stated, clean check is temporal above).
4. **Published benchmarks:** `benchmarks.md:5` — Dibang `0.96` → `best 0.9029` below (honest), Meghalaya `>90%` → `82.08%` below, `threshold screen` consistency only.
...
```text
ngen outputs: data/sih26001/processed/feature_matrix.training.csv (2936×22, 1468+1468, git-ignored) + data/sih26001/evidence/feature_matrix.training.sample.csv (20 rows, committed) + data/sih26001/manifest.training.json (committed)
models: ml/models/sih26001_{rf,xgb,lgb,lr,iso}_v1.joblib (git-ignored, sha256 in manifest)
reports: ml/sih26001/reports/{metrics,calibration,benchmarks}.md (committed) — RF OOF 0.8983 XGB 0.9029 LGBM 0.9015, cal Brier 0.118, temporal test AUC 0.8189 (673/73 dated)
model card: docs/sih26001/ML_MODEL_CARD_V2.md (committed draft, clean=true)
```

Built: `scripts/build_training_matrix.py:1` 2936 rows + `scripts/train_sih26001.py:1` RF500/XGB/LGBM + SHAP 5-pt `manifest.training.json:shap_sample` on `mnemo` venv (`xgb 3.2/lgbm 4.7/shap 0.51`).
