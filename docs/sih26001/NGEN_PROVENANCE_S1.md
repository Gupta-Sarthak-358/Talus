...
*   ✅ Lineament (PROXY-Bhuvan/figure, 2026-09-04): ISRO/NRSC Bhuvan Lineament 50K 2005-06 Sikkim available-state advertised (`bhuvan-app1.nrsc.gov.in/mines/mines.php` + `nwdp.nwic.in/dataset/lineament`: "in association with GSI, satellite + collateral, limited field work"), WMS `bhuvan-vec2.nrsc.gov.in/bhuvan/wms` timed out from this machine (Bhuvan Wiki layer naming needs QGIS GetCapabilities browse) + report Figures 24/47 Lineament Buffer/Density Maps Gangtok (NESAC, same report) show central Gangtok low-moderate (<1.5) → `scripts/extract_lineament.py` → `s234_lineament.json` (sha256 in manifest): **0.8 km/km2** all slopes — conservative literature proxy (Gangtok-adjacent 0.3-1.4 at 50K, uniform because figure not per-slope digitized). Tag PROXY-published-map + Bhuvan-availability. Upgrade: Bhuvan Thematic "Clip and Ship" Shapefile → length/area per slope.
