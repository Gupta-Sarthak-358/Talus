...
2. **Inventory incompleteness** — `672/764` pos `year 0` undated `training_sidecar.csv:1`; `92→108 dated` after rescue `61` clusters `build_training_matrix.py:294`; small slides unreported — candidate sidecar `reports.json:1` is the mitigation.
...
9. **Prototype bands ≠ safety standards.** Operational decisions remain with
 qualified authorities — `calibrated Brier 0.118` same-OOF optimism `calibration.md:3`, clean check is temporal `AUC 0.8189`.
