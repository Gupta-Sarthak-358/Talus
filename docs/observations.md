...
## 25. FR-03 calibration + Evidence Timeline + demo re-freeze + system doc

- **Calibration (FR-03):** ml/benchmark/calibration.py -- isotonic P(score>=75) fit on OUT-OF-FOLD train-seed predictions (GroupKFold k=5 by seed); evaluated on validation seeds only. Brier 0.0812 vs 0.116 naive (-30%); ECE 0.095 vs 0.157. High-risk bins honest (pred 0.93 vs obs 0.98); mid-bins overconfident on sparse data (documented). Artifact: ml/models/talus_calibration_v1.joblib. API confidence now = calibrated probability of elevated synthetic risk (meaning documented; NOT probability of rockfall).
- **Integration bug found by test:** /api/risk/predict returned stale bootstrapped confidence after recompute; recompute() now refreshes score+confidence together.
- **Evidence Timeline (FR-10):** scenario_service.evidence_timeline() derives cause-attributed events from trajectory state changes (rain jumps, gw rise, severity transitions, blast events, water-filled activation) -- distinct from SHAP by design. Exposed in causal-what-if response.
- **missing_evidence:** derived from feature provenance (attenuation-modeled PPV, rainfall-derived gw proxy, sampler crack states) instead of hardcoded string.
- **Causal summary extended:** fos_divergence_min / divergence_day / days_diverging_gt_001 now exposed through the API (the -0.761 multi-year result is no longer invisible).
- **Demo re-frozen:** docs/06_DEMO_SCENARIO.md v2 against real model + calibration. Key discoveries: Zone B saturated at 100 (old escalation narrative impossible -> protagonist is Zone C at 66 Moderate); ML what-if on correlated features moves off-manifold (gw ladder LOWERS C score 66->58 despite positive physics) -- live demonstration of why causal simulation is a separate path.
- **docs/CURRENT_SYSTEM.md:** single source of truth; superseded/historical docs explicitly marked.
- Backend suite: 25/25 PASS.
